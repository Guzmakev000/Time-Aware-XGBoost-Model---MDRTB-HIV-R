...
for (n_weeks in MODEL_WEEKS) {
  ...
  w_ipcw[!is.finite(w_ipcw)] <- NA_real_
  cap <- stats::quantile(w_ipcw, 0.95, na.rm = TRUE)
  w_ipcw[w_ipcw > cap] <- cap
  ...
  if (!is.na(p_m) && p_m > 0) w_sex[sex_kept == 0] <- (MALE_TARGET   / p_m) ^ 1
  if (!is.na(p_f) && p_f > 0) w_sex[sex_kept == 1] <- (FEMALE_TARGET / p_f) ^ 1
